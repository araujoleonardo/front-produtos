  ...
  <div>
    <el-card class="box-card">
      ...
      <el-table :data="dataSet.data" v-loading="loading" style="width: 100%" border>
        <el-table-column label="Categoria" prop="name" sortable/>
...
<script setup lang="ts">
  ...
  const currentUrl = ref<string>('/category?page=1');
  ...
  const getData = (url: string): void => {
    ...
    api.get(currentUrl.value, {
      ...
      dataSet = response.data.category;
    }).catch((error) => {
  ...
  const handleDelete = (index: number, row): void => {
    ElMessageBox.confirm(
      ...
      .then(() => {
        ...
        api.delete(`/category-delete/${row.id}`, {
          headers:{
            Authorization: tokenAuth
          }
        }).then((response) => {
  ...
  const btnPesquisar = (): void => {
    const url = `/category?pesquisa=${search.value}`;
    getData(url);
  };
</script>
<style scoped>
